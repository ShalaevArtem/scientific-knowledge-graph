...
- [x] Ручная корректировка графа экспертом: из очереди needs_review можно подтвердить,
      поправить (имя/синонимы/текст/география), прокомментировать или удалить ошибочное
      извлечение; автор и дата фиксируются на узле (`reviewed_by`/`reviewed_at`) и в аудите
      (`POST /api/review/{approve|edit|comment|reject}`)
- [x] Дашборд руководителя: покрытие знаний по направлениям, зоны риска (слабо освещённые
      пары материал×процесс), прогресс верификации, носители экспертизы (`/api/dashboard`,
      вкладка «Дашборд»; внутренние роли)
