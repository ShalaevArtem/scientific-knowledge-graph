...
Архитектура — **GraphRAG**: граф знаний (скелет связей) + гибридный семантический поиск
(вектор + BM25) + LLM-слой для вопросов на естественном языке с цитатами.
...
```
Корпус (PDF/DOCX/PPTX)                  ~1 ГБ: доклады, журналы (2003–2026),
        │                                конференции, обзоры, статьи (RU/EN)
        ▼  skg ingest
Document + Chunk в графе                метаданные из структуры папок,
        │                                текст с привязкой к страницам/слайдам
        ├──▼  skg embed
...
| `skg extract [--limit N] [--doc ID]` | LLM-извлечение из чанков (возобновляемо) |
...
src/skg/
  ingest/          PDF/DOCX/PPTX → текст → чанки; реестр корпуса из структуры папок
...
- [x] UI: поиск с ответом, граф-эксплорер, тепловая карта пробелов
- [ ] Версионирование фактов и связи SUPPORTS/CONTRADICTS между выводами из текстов
- [ ] RBAC, аудит, экспорт (PDF/JSON-LD), уведомления — см. презентацию
