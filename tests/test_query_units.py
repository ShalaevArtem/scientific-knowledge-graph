...
def test_answer_question_explicit_filters_override_llm_filters(monkeypatch):
    ...
    # графовые каналы поиска требуют Neo4j — в юнит-тесте отключаем
    monkeypatch.setattr(answer_mod, "resolve_question_entities", lambda session, q: [])
    monkeypatch.setattr(answer_mod, "measurement_candidates", lambda session, c: [])
